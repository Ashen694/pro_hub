...
class PaidTraineesExport implements FromCollection, WithHeadings, WithMapping, WithStyles, WithColumnWidths
{
    /**
    ...
    {
        return Trainee::paid()->orderBy('Trainee_ID', 'desc')->get();
    }
    ...
    public function map($trainee): array
    {
        return [
            ...
            $trainee->requested_payment_date ? $trainee->requested_payment_date->format('Y-m-d') : '-',
            $trainee->absent_Count ?? '0',
            ucfirst($trainee->status ?? 'paid'),
        ];
    }
