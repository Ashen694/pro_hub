...
@section('page-title', 'External Solution Issues')
...
@push('styles')
    <style>
        ...
        .action-btn {
            ...
            border: none; cursor: pointer; margin: 0 2px;
        }
...
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
        <h3 class="card-title mb-0">Issues Reported on External Solutions</h3>
        <a href="{{ route('incidents.external.create') }}" class="btn btn-primary">+ Create New Issue</a>
    </div>
    <div class="card-body">
        @if(session('success'))
            ...
            <table class="table table-bordered table-hover align-middle">
                ...
                <tbody>
                    @forelse ($issues as $issue)
                        <tr>
                            <td>EXT-{{ str_pad($issue->ID, 4, '0', STR_PAD_LEFT) }}</td>
                            <td>{{ \Carbon\Carbon::parse($issue->Issue_Start_Time)->format('Y-m-d h:i A') }}</td>
                            <td>{{ $issue->platform_name }}</td>
                            ...
                            <td class="text-center d-flex justify-content-center">
                                ...
                                <button type="button" class="action-btn action-btn-view" data-bs-toggle="modal" data-bs-target="#issueDetailsModal" 
                                    data-ref-id="EXT-{{ str_pad($issue->ID, 4, '0', STR_PAD_LEFT) }}"
                                    data-start-time="{{ \Carbon\Carbon::parse($issue->Issue_Start_Time)->format('F d, Y h:i A') }}"
                                    data-platform="{{ $issue->platform_name }}"
                                ...
                                <!-- Edit Button -->
                                <a href="{{ route('incidents.external.edit', $issue->ID) }}" class="action-btn action-btn-edit">
                                ...
                                <button type="button" class="action-btn action-btn-delete" 
                                    ...
                                    data-delete-url="{{ route('incidents.external.destroy', $issue->ID) }}">
                                    <i class="ti ti-trash"></i>
                                </button>
                            </td>
                        </tr>
                    @empty
                        <tr><td colspan="7" class="text-center">No external issues found.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
