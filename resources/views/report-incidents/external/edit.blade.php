...
@section('page-title', 'Edit External Issue')

@section('content')
<div class="card">
    <div class="card-header">
        <h3 class="card-title">Edit Issue - EXT-{{ str_pad($issue->ID, 4, '0', STR_PAD_LEFT) }}</h3>
    </div>
    <div class="card-body">
        @if ($errors->any())
        ...
        @endif
        <form action="{{ route('incidents.external.update', $issue->ID) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="row">
                ...
                <div class="col-md-12 mb-3">
                    ...
                    <select name="Platform_ID" class="form-select" required>
                        ...
                        @foreach($platforms as $platform)
                            <option value="{{ $platform->platform_id }}" {{ old('Platform_ID', $issue->Platform_ID) == $platform->platform_id ? 'selected' : '' }}>
                ...
                <div class="col-md-12 mb-3">
                    ...
                    <select name="Reported_By" class="form-select" required>
                         ...
                         @foreach($contacts as $contact)
                            <option value="{{ $contact->name }}" {{ old('Reported_By', $issue->Reported_By) == $contact->name ? 'selected' : '' }}>
                                {{ $contact->name }}
                            </option>
                         @endforeach
                    </select>
                ...
                <div class="col-md-12 mb-3">
                    ...
                    <input type="text" class="form-control" value="{{ $issue->Entered_By }}" disabled style="cursor: not-allowed; background-color: #e9ecef;">
                </div>
            ...
            <div class="form-footer text-center">
                <a href="{{ route('incidents.external.index') }}" class="btn btn-secondary">Cancel</a>
