@extends('layouts.app')
@section('page-title','Create Customer Contact')
@section('content')
<style>
...
<div class="container">
    <div class="form-container">
        <h4 class="mb-4">Create New Customer Contact</h4>
        
        @if(session('success'))
            <div class="alert alert-success">{{ session('success') }}</div>
        @endif
        
        <form action="{{ route('reference-data.customer-contacts.store') }}" method="POST">
            @csrf
            ...
            <div class="mb-3">
                ...
                <select name="external_platform_id" id="external_platform_id" class="form-control @error('external_platform_id') is-invalid @enderror">
                    ...
                    @foreach($externalPlatforms as $platform)
                        <option value="{{ $platform->platform_id }}" {{ old('external_platform_id') == $platform->platform_id ? 'selected' : '' }}>
